...
FROM tblit_request WHERE TO_CHAR(prepared_by_date, 'YYYY-MM') = '{$month}' AND request_type = '{$requestType}' AND repaired_by = '{$user}' ORDER BY queue_number ASC";
...
FROM tblit_request WHERE TO_CHAR(prepared_by_date, 'YYYY-MM') <> '{$month}' AND request_type = '{$requestType}' AND status <> 'Done' AND repaired_by = '{$user}' ORDER BY queue_number ASC";
    ...
    public function Header()
    {
        ...
        if ($this->PageNo() == 1) {
            ...
            if ($requestType == 'Hardware') {
                ...
                $this->Cell(0, 0, 'SOFTWARE DEVELOPMENT NEW REQUEST', 0, 1, 'C');
            }
        }
        if ($this->PageNo() >= 2) {
            ...
            if ($this->PageNo() == 5) {
                if ($requestType == 'Hardware') {
                    ...
                    $this->Cell(304.5, 5, 'Software Development New Request - ' . date('F Y', strtotime($month)), 0, 0, 'L');
                }
            } else {
                if ($requestType == 'Hardware') {
                    $this->Cell(304.5, 5, 'Hardware Request - ' . date('F Y', strtotime($month)), 0, 0, 'L');
                        ...
                        $html_header = '
                            <table cellspacing="0" cellpadding="0.2" border="1" style="width:100%">
                                <tr>
                                    ...
                                    <th align="center" width="14%"><div style="font-size:5pt">&nbsp;</div>Control No.</th>
                                    <th align="center" width="14%"><div style="font-size:5pt">&nbsp;</div>Requested By</th>
                                    <th align="center" width="26%"><div style="font-size:5pt">&nbsp;</div>Purpose</th>
    ...
    public function OutputTable($itemData_List)
    {
        ...
        if ($requestType == 'Hardware') {
            foreach ($itemData_List as $details) {
                ...
                $html .= '<td style="width: 14%;">' . $details['queue_number'] . '</td>';
                $html .= '<td style="width: 14%;">' . $details['prepared_by'] . '</td>';
                $html .= '<td style="width: 26%;">' . $details['purpose'] . '</td>';
...
$pdf->Cell(0, 57, strtoupper(date('F Y', strtotime($month))), 0, 0, 'C');
$pdf->Ln(39.5);
...
$pdf->Ln();
//* -------------------------------- Job Position --------------------------------
$pdf->Cell(76.5, 5, getJobPosition($php_fetch_bannerweb_api, $user), 0, 0, 'L', 0, '', 0, false, 'T', 'M');
