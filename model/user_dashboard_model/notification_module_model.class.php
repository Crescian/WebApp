...
class notificationModule
{
    ...
    public function load_nav_link($BannerWeb, $fullname)
    {
        $categoryArray = array();
        $sqlstring = "SELECT DISTINCT app_name,app_id FROM bpi_notification_module 
        INNER JOIN bpi_app_menu ON bpi_app_menu.appid = bpi_notification_module.app_id WHERE prepared_by = ? OR approved_by = ? OR noted_by = ?
        ORDER BY app_name DESC";
        $result_stmt = $BannerWeb->prepare($sqlstring);
        $result_stmt->execute([$fullname, $fullname, $fullname]);
    ...
    public function load_assignatory($BannerWeb, $id, $fullname)
    {
        $categoryArray = array();
        $sqlstring = "SELECT app_id,table_name,(Case when prepared_by = '{$fullname}' THEN 1 ELSE 0 END) as prepared_by,
    ...
    public function loadCount($BannerWeb, $app_id, $fullname)
    {
        ...
        FROM bpi_notification_module
        WHERE (approved_by_acknowledge = true OR checked_by_acknowledge = true) AND noted_by_acknowledge = false AND app_id = '{$app_id}' AND noted_by = '{$fullname}';";
        $stmt = $BannerWeb->prepare($query);
        $stmt->execute();
        $rowData = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $rowCount = $stmt->rowCount();
    ...
    public function loadTable($BannerWeb, $app_id, $fullname, $label)
    {
        // //* ======== Prepare Array ========
        //* ======== Read Data ========
        ...
        //* ======== Fetch Record ========
        $sqlstring = "SELECT * FROM bpi_notification_module WHERE app_id = '{$app_id}' AND $label = '{$fullname}' AND prepared_by_acknowledge = false ORDER BY notificationid ASC";
        $result_stmt = $BannerWeb->prepare($sqlstring);
        $result_stmt->execute();
        $result_total_record = $result_stmt->rowCount();
        //* ======== Fetch Total Filtered Record ========
        $sqlstring = "SELECT * FROM bpi_notification_module WHERE 1 = 1 AND app_id = '{$app_id}' AND $label = '{$fullname}' AND prepared_by_acknowledge = false";
        //* ======== Search ========
