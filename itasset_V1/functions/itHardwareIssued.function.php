...
$WHPO = $conn->db_conn_warehouse(); //* Physical Security Database connection
$ITA = $conn->db_conn_itassetExtention(); //* Physical Security Database connection
session_start();
...
if (isset($_POST['action'])) {
    ...
    switch ($action) {
        case 'loadTableHardwareIssuance':
            ...
            $query = "SELECT hardware_issuance_employee_id, tblit_cpu_control_no.employee, employee_barcode, tblit_cpu_control_no.cpu_control_no, tblit_hardware_issuance_employee.description, issued_by, date_issued, status 
                        FROM tblit_hardware_issuance_employee 
                      INNER JOIN tblit_cpu_control_no 
                        ON tblit_cpu_control_no.cpu_control_no_id = tblit_hardware_issuance_employee.cpu_control_no_id
        ...
        case 'loadTableMachineIssuance':
            ...
            if (!empty($search)) {
                $query .= "AND (machine ILIKE '%{$search}%'
                    ...
                    OR issued_by.description ILIKE '%{$search}%'
