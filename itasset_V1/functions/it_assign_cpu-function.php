    ...
    switch ($action) {
        case 'loadTableAssignCPU':
            ...
            foreach ($rowData as $row) {
                $data[] = array(
                    ...
                    $row['ip_address'],
                    $row['date_updated'],
                    '<button class="btn btn-danger" onclick="editAssignCPU(' . $row['cpu_control_no_id'] . ');" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Edit"><i class="fa-solid fa-file-pen"></i></button><button class="btn btn-dark" onclick="pdfAssignCPU(' . $row['cpu_control_no_id'] . ');" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Print"><i class="fa-solid fa-file-pdf"></i></button>'
                );
            }
        ...
        case 'newAssignCPU':
            ...
            $query = "INSERT INTO tblit_cpu_control_no(active_pc, cpu_control_no, employee, description, location, switch_tag, lan_cable_tag, ip_address, date_updated)
                        VALUES(true, '{$cpuControlNumber}', '{$employee}', '{$description}', '{$location}', '{$switchTag}', '{$lanCableTag}', '{$ipAddress}', '{$dateToday}')";
            $stmt = $ITA->prepare($query);
            $stmt->execute();
